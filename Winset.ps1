...
$system_actions = @{
	...
	'Operating System Version' = 'Get-ItemProperty "HKLM:\Software\Microsoft\Windows NT\CurrentVersion\" | select-object ProductName,Displayversion,WinREversion';
...
$priviliges_actions = @{
	...
	'Program Files Folder Permission' = 'Get-Acl -Path "C:\Program Files"';
	'ProgramData Folder Permission' = 'Get-Acl -Path "C:\ProgramData"';
...
Add-Content -Path $OutputFile -Value "<pre><span>Domain:</span>$(wmic computersystem get domain)</pre></div></div>" #can also try: $env:USERDNSDOMAIN
